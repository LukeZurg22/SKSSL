...
The engine configuration is through the `EngineConfig` class, which can be inherited and provided new fields.
The Config used to be loaded within the static constructor, but is now loaded through the virtual function
`BuildEngineConfig()`. Override this function to return your own configuration class.

```csharp
public class MyGame : SSLGame
{
    /*Replace the "EngineConfig type with your own inheriting type.*/
    public override EngineConfig BuildEngineConfig() => new EngineConfig()
    {
...
By default this requires the ECS to be enabled! The PrototypeLoader class is an abstract inheritable class that allows a
developer to create their own loader that
