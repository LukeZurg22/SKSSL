...
/// </summary>
public class ExampleImGUIGame : Game
{
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        if (WasResized)
        {
            ...
            debug_log.Add("(" + DateTime.Now.ToShortTimeString() + ") [MonoGame] Changed Resolution to " +
    ...
    private void DrawImGuiOverlay(float frameRate)
    {
        ...
        if (ImGui.Begin("Example: Simple overlay", windowFlags))
        {
            ...
            ImGui.Text($"Frames per second: {frameRate.ToString(CultureInfo.InvariantCulture)}");
        ...
        if (ImGui.BeginMenu("Colors"))
        {
            ...
            for (int i = 0; i < (int)ImGuiCol.COUNT; i++)
            {
                ...
                ImGui.GetWindowDrawList()
                    .AddRectFilled(p, new Vec2(p.X + sz, p.Y + sz), ImGui.GetColorU32((ImGuiCol)i));
    ...
    unsafe private struct ImGuiDebugLog
    {
        ...
        public void Draw(string title)
        {
            ...
            ImGui.Separator();
            ImGui.BeginChild("scrolling", new Vec2(0, 0), ImGuiChildFlags.None,
    ...
    unsafe private struct ImGuiExampleAppConsole
    {
        ...
        public void Draw(string title)
        {
            ...
            ImGui.BeginChild("ScrollingRegion", new Vec2(0, -footer_height_to_reserve), ImGuiChildFlags.None,
                ImGuiWindowFlags.HorizontalScrollbar);
            if (ImGui.BeginPopupContextWindow())
            {
            ...
            ImGuiInputTextFlags
                input_text_flags =
                    ImGuiInputTextFlags
                        .EnterReturnsTrue; //| ImGuiInputTextFlags.CallbackCompletion | ImGuiInputTextFlags.CallbackHistory;
    ...
    private void DrawDemoWindow()
    {
        ...
        //Main body
        if (!ImGui.Begin("Dear ImGui Demo / Monogame & ImGui.Net",
                windowFlags)) //ref p_open, windowFlags)) Close button is not changing bool!?
        {
            ImGui.End();
            return;
        }
    ...
    private void DemoWindowWidgets()
    {
        ...
        if (ImGui.TreeNode("Trees"))
        {
            ...
            if (ImGui.TreeNode("Advanced, with Selectable nodes"))
            {
                ...
                ImGui.CheckboxFlags("ImGuiTreeNodeFlags_OpenOnArrow", ref base_flags,
                    (uint)ImGuiTreeNodeFlags.OpenOnArrow);
                ImGui.CheckboxFlags("ImGuiTreeNodeFlags_OpenOnDoubleClick", ref base_flags,
                    (uint)ImGuiTreeNodeFlags.OpenOnDoubleClick);
        ...
        if (ImGui.TreeNode("Text"))
        {
            ...
            if (ImGui.TreeNode("Word Wrapping"))
            {
                ...
                for (int n = 0; n < 2; n++)
                {
                    ...
                    {
                        ImGui.Text(
                            $"The lazy dog is a good dog. This paragraph should fit within {wrap_width} pixels. Testing a 1 character word. The quick brown fox jumps over the lazy dog.");
                    }
                    else
                    {
                        ImGui.Text(
                    ...
                    uint col2 = ImGui
                        .ColorConvertFloat4ToU32(colf2); //ImGuiNative.igColorConvertFloat4ToU32(colf2);
        ...
        if (ImGui.TreeNode("Combo"))
        {
            ...
            if (ImGui.CheckboxFlags("ImGuiComboFlags_NoArrowButton", ref flags,
                    (uint)ImGuiComboFlags.NoArrowButton))
            {
        ...
        if (ImGui.TreeNode("Selectables"))
        {
            if (ImGui.TreeNode("Basic"))
            {
                ...
                if (ImGui.Selectable("5. I am double clickable", ref selection[4],
            ...
            {
                ImGui.Selectable("main.c", ref selected_rend[0]);
                ImGui.SameLine(300);
                ImGui.Text(" 2,345 bytes");
                ImGui.Selectable("Hello.cpp", ref selected_rend[1]);
                ImGui.SameLine(300);
                ImGui.Text("12,345 bytes");
                ImGui.Selectable("Hello.h", ref selected_rend[2]);
                ImGui.SameLine(300);
                ImGui.Text(" 2,345 bytes");
                ImGui.TreePop();
            }
            ...
            if (ImGui.TreeNode("Alignment"))
            {
                ...
                for (int y = 0; y < 3; y++)
                {
                    for (int x = 0; x < 3; x++)
                    {
                        ...
                        ImGui.Selectable(name, ref selected_align[3 * y + x], ImGuiSelectableFlags.None,
                            new Vec2(80, 80));
                        ImGui.PopStyleVar();
                    }
                }
        ...
        if (ImGui.TreeNode("Text Input"))
        {
            if (ImGui.TreeNode("Multi-line Text Input"))
            {
                ...
                ImGui.CheckboxFlags("ImGuiInputTextFlags_ReadOnly", ref flags_ti,
                    (uint)ImGuiInputTextFlags.ReadOnly);
                ImGui.CheckboxFlags("ImGuiInputTextFlags_AllowTabInput", ref flags_ti,
                    (uint)ImGuiInputTextFlags.AllowTabInput);
                ImGui.CheckboxFlags("ImGuiInputTextFlags_CtrlEnterForNewLine", ref flags_ti,
                    (uint)ImGuiInputTextFlags.CtrlEnterForNewLine);

                ImGuiInputTextFlags flags_ml = (ImGuiInputTextFlags)flags_ti;
                ImGui.InputTextMultiline("##source", ref text, (uint)text.Length,
                    new Vec2(0, ImGui.GetTextLineHeight() * 16), flags_ml);
                ImGui.TreePop();
            }
        ...
        if (ImGui.TreeNode("Tabs"))
        {
            ...
            if (ImGui.TreeNode("Advanced & Close Button"))
            {
                ...
                ImGui.CheckboxFlags("ImGuiTabBarFlags_Reorderable", ref tab_bar_flags,
                    (uint)ImGuiTabBarFlags.Reorderable);
                ImGui.CheckboxFlags("ImGuiTabBarFlags_AutoSelectNewTabs", ref tab_bar_flags,
                    (uint)ImGuiTabBarFlags.AutoSelectNewTabs);
                ImGui.CheckboxFlags("ImGuiTabBarFlags_TabListPopupButton", ref tab_bar_flags,
                    (uint)ImGuiTabBarFlags.TabListPopupButton);
                ImGui.CheckboxFlags("ImGuiTabBarFlags_NoCloseWithMiddleMouseButton", ref tab_bar_flags,
                ...
                if (ImGui.CheckboxFlags("ImGuiTabBarFlags_FittingPolicyResizeDown", ref tab_bar_flags,
                        (uint)ImGuiTabBarFlags.FittingPolicyResizeDown))
                {
                    tab_bar_flags &= ~((uint)ImGuiTabBarFlags.FittingPolicyMask ^
                                       (uint)ImGuiTabBarFlags.FittingPolicyResizeDown);
                }

                if (ImGui.CheckboxFlags("ImGuiTabBarFlags_FittingPolicyScroll", ref tab_bar_flags,
        ...
        if (ImGui.TreeNode("Color/Picker Widgets"))
        {
            ...
            ImGui.ColorEdit4("MyColor##3", ref color_vec4,
                ImGuiColorEditFlags.NoInputs | ImGuiColorEditFlags.NoLabel | misc_flags);
            ...
            ImGui.Combo("Picker Mode", ref picker_mode,
            ...
            ImGui.ColorPicker4("MyColor##4", ref color_vec4, flags,
                ref ref_color_v.X); // ref_color ? ref_color_v.X : null);
            ...
            {
                ImGui.SetColorEditOptions(ImGuiColorEditFlags.Uint8 | ImGuiColorEditFlags.DisplayHSV |
            ...
            ImGui.ColorEdit4("HSV shown as RGB##1", ref color_hsv,
                ImGuiColorEditFlags.DisplayRGB | ImGuiColorEditFlags.InputHSV | ImGuiColorEditFlags.Float);
            ImGui.ColorEdit4("HSV shown as HSV##1", ref color_hsv,
        ...
        {
            ImGui.DragFloatRange2("range float", ref begin, ref end, 0.25f, 0.0f, 100.0f, "Min: %.1f %%",
                "Max: %.1f %%");
            ImGui.DragIntRange2("range int", ref begin_i, ref end_i, 5, 0, 1000, "Min: %d units", "Max: %d units");
            ImGui.DragIntRange2("range int (no bounds)", ref begin_i, ref end_i, 5, 0, 0, "Min: %d units",
                "Max: %d units");
            ImGui.TreePop();
        }
    ...
    private void DemoWindowLayout()
    {
        ...
        if (ImGui.TreeNode("Child windows"))
        {
            ...
            ImGui.BeginChild("ChildL", new Vec2(ImGui.GetWindowSize().X * 0.5f, 260), ImGuiChildFlags.Borders,
                window_flags);
            for (int i = 0; i < 100; i++)
            {
            ...
            ImGui.Text(string.Format("Rect of child window is: ({0},{1}) ({2},{3})", child_rect_min.X,
        ...
        if (ImGui.TreeNode("Basic Horizontal Layout"))
        {
            ...
            // Checkbox
            ImGui.Checkbox("My", ref c1);
            ImGui.SameLine();
            ImGui.Checkbox("Tailor", ref c2);
            ImGui.SameLine();
            ImGui.Checkbox("Is", ref c3);
            ImGui.SameLine();
            ImGui.Checkbox("Rich", ref c4);

            // Various
            ImGui.PushItemWidth(80);
            ImGui.Combo("Combo", ref item, items, items.Length);
            ImGui.SameLine();
            ImGui.SliderFloat("X", ref bf0, 0.0f, 5.0f);
            ImGui.SameLine();
            ImGui.SliderFloat("Y", ref bf1, 0.0f, 5.0f);
            ImGui.SameLine();
            ...
            float window_visible_x2 = ImGui.GetWindowPos().X + ImGui.GetWindowSize().X;
            for (int n = 0; n < buttons_count; n++)
            {
                ...
                float next_button_x2 =
                    last_button_x2 + 1.0f + button_sz.X; // Expected position if next button was on same line
        ...
        ImGui.SameLine(140);
        enable_track |= ImGui.DragInt("##item", ref track_item, 0.25f, 0, 99, "Item = %d");

        bool scroll_to_off = ImGui.Button("Scroll Offset");
        ImGui.SameLine(140);
        scroll_to_off |= ImGui.DragFloat("##off", ref scroll_to_off_px, 1.00f, 0, 100.0f, "+%.0f px");

        bool scroll_to_pos = ImGui.Button("Scroll To Pos");
        ImGui.SameLine(140);
        scroll_to_pos |= ImGui.DragFloat("##pos", ref scroll_to_pos_px, 1.00f, -10, 100.0f, "X/Y = %.0f px");
        ImGui.PopItemWidth();
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            bool child_is_visible = ImGui.BeginChild(child_id, new Vec2(child_w, 200.0f), ImGuiChildFlags.Borders,
                child_flags);
            if (ImGui.BeginMenuBar())
            {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ImGuiWindowFlags child_flags = ImGuiWindowFlags.HorizontalScrollbar |
    ...
    private void DemoWindowPopups()
    {
        ...
        {
            ImGui.Text($"Value = {value} (<-- right-click here)");
        ...
        if (ImGui.TreeNode("Modals"))
        {
            ...
            if (ImGui.BeginPopupModal("Delete?", ref show, ImGuiWindowFlags.AlwaysAutoResize))
            {
                ...
                if (ImGui.Button("OK", new Vec2(120, 0)))
                {
                    ImGui.CloseCurrentPopup();
                }
        ...
        if (ImGui.CollapsingHeader("Inputs, Navigation & Focus"))
        {
            ...
            if (ImGui.TreeNode("Keyboard, Mouse & Navigation State"))
            {
                ...
                for (int i = 0; i < io.MouseDown.Count; i++)
                    ...
                    {
                        ImGui.SameLine();
                        ImGui.Text($"{i} ({io.MouseDownDuration[i]} secs)");
                    }
                ...
                for (int i = 0; i < io.MouseDown.Count; i++)
                    if (ImGui.IsMouseDoubleClicked((ImGuiMouseButton)i))
                    {
                        ImGui.SameLine();
                        ImGui.Text(i.ToString());
                    }
                ...
                for (int i = 0; i < io.MouseDown.Count; i++)
                    if (ImGui.IsMouseReleased((ImGuiMouseButton)i))
                    {
                        ImGui.SameLine();
                        ImGui.Text(i.ToString());
                    }
