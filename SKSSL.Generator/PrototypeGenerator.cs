...
public class PrototypeGenerator : IIncrementalGenerator
{
    ...
    private static StringBuilder GenerateOutputFile(List<INamedTypeSymbol> prototypes)
    {
        ...
        sb.AppendLine("            SKSSL.ECS.ECSMasterRegistry.Clear();");
        // Accommodating base-entity type, which can exist out in the wild. Base-prototype, however, cannot!
        sb.AppendLine($"            SKSSL.ECS.ECSMasterRegistry.RegisterTypeDefinition(\"Entity\", typeof(global::SKSSL.ECS.Entity));");
        foreach (INamedTypeSymbol? proto in prototypes)
        {
            ...
            sb.AppendLine($"            SKSSL.ECS.ECSMasterRegistry.RegisterTypeDefinition(\"{shortName}\", typeof({fullName}));");
        }
        sb.AppendLine("      }");
        sb.AppendLine("}");
        return sb;
